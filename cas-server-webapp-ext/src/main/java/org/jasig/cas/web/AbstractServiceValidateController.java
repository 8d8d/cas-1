...
import com.gzzq.config.WebConfigHelper;
...
//    	if(!assertion.isFromNewLogin()){//不是第一次登录，不送积分
//    		return;
//    	}
    	String jsonFormat ="{\"userUuid\":\"%s\",\"account\":\"%s\",\"operateTime\":\"%s\",\"platform\": \"WEB\",\"eventCode\":\"E1001\",\"bizName\":\"商城PC端登录\",\"bizVal\": \"SHOP_WEB\"}";
    	Principal principal= assertion.getPrimaryAuthentication().getPrincipal();
    	String result = String.format(jsonFormat, principal.getAttributes().get("uuid"),principal.getAttributes().get("account"),assertion.getPrimaryAuthentication().getAuthenticationDate());
    	String resultPrint = HttpClientUtil.postJSON(WebConfigHelper.getLoginIntegralUrl(), result);
    }
